...
- `make up` failing with "port is already allocated": another project is using 5432/6379/8000 on your host. Either stop it or drop/repoint the `ports:` mappings in `docker/docker-compose.dev.yml` (only `api` needs a host port for local use).
