...
- `go test -tags=integration ./...` failing with `rootless Docker not found, failed to create Docker provider`: testcontainers looks for the daemon at `/var/run/docker.sock` and colima does not put it there. Point it at colima's socket:
  ```bash
  export DOCKER_HOST="unix://$HOME/.colima/default/docker.sock"
  export TESTCONTAINERS_DOCKER_SOCKET_OVERRIDE=/var/run/docker.sock
  ```
  (The second variable is what testcontainers tells *containers* to mount, and must stay the canonical path.) Docker Desktop needs neither.
- `go test -tags=integration ./...` failing with `connection reset by peer` against Postgres: a container was started without `postgres.BasicWaitStrategies()`. The official image runs initdb against a temporary server, logs "ready to accept connections", then shuts it down and starts the real one — connecting in that window is refused. Every Postgres container in this repo passes that option; new ones must too.
