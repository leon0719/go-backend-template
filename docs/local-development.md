...
- `make migrate` / `make migrate-down` — apply/roll back goose migrations via the `goose` CLI (must be installed on your host) against `$DATABASE_URL`; when running against the dev stack's Postgres, export `DATABASE_URL=postgres://postgres:postgres@localhost:5433/go_backend_template?sslmode=disable` first (the dev stack maps Postgres to host port 5433 to avoid colliding with a locally-installed Postgres on 5432)
...
- `make up` failing with "port is already allocated": another project is using 5433/6380/8000 on your host. Postgres and Redis are deliberately mapped to 5433/6380 rather than their standard 5432/6379 so they don't collide with locally-installed instances — but if something else already holds them, stop it or drop/repoint the `ports:` mappings in `docker/docker-compose.dev.yml` (only `api` needs a host port for local use).
